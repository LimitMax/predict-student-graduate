...
with st.form("student_form"):
    ...
    with col1:
        marital_status = st.selectbox("Status Pernikahan", ["Pilih", 'Single', 'Married', 'Widower', 'Divorced','Facto Union','Legally Separated', 'Other'])

        marital_status_placeholder = st.empty()

        if marital_status == 'Other':
            marital_status_other = marital_status_placeholder.text_input("↳ Tulis Status Pernikahan Lainnya")
        else:
            marital_status_other = None

        application_mode = st.selectbox("Mode Aplikasi", ["Pilih", '1st phase - general contingent', 'Ordinance No. 612/93','1st phase - special contingent (Azores Island)','Holders of other higher courses','Ordinance No. 854-B/99','International student (bachelor)','1st phase - special contingent (Madeira Island)','2nd phase - general contingent','3rd phase - general contingent','Ordinance No. 533-A/99, item b2) (Different Plan)','Ordinance No. 533-A/99, item b3 (Other Institution)','Over 23 years old','Transfer','Change of course','Technological specialization diploma holders','Change of institution/course','Short cycle diploma holders','Change of institution/course (International)', 'Other'])
        course = st.selectbox("Program Studi", ["Pilih", 'Biofuel Production Technologies','Animation and Multimedia Design','Social Service (evening attendance)','Agronomy','Communication Design','Veterinary Nursing','Informatics Engineering','Equinculture','Management','Social Service','Tourism','Nursing','Oral Hygiene','Advertising and Marketing Management','Journalism and Communication','Basic Education','Management (evening attendance)', 'Other'])
        attendance = st.selectbox("Waktu Kehadiran", ["Pilih", 'Daytime', 'Evening', 'Other'])

    with col2:
        prev_qualification = st.selectbox("Kualifikasi Sebelumnya", ["Pilih", "Secondary education","Higher education - bachelor's degree","Higher education - degree","Higher education - master's","Higher education - doctorate","Frequency of higher education","12th year of schooling - not completed", "11th year of schooling - not completed","Other - 11th year of schooling","10th year of schooling","10th year of schooling - not completed","Basic education 3rd cycle (9th/10th/11th year) or equiv.","Basic education 2nd cycle (6th/7th/8th year) or equiv.","Technological specialization course","Higher education - degree (1st cycle)","Professional higher technical course","Higher education - master (2nd cycle)", "Other"])
        prev_qual_grade = st.number_input("Nilai Kualifikasi Sebelumnya", min_value=0.0, max_value=200.0, value=0.0)
        admission_grade = st.number_input("Nilai Masuk (Admission Grade)", min_value=0.0, max_value=200.0, value=0.0)
        age = st.number_input("Umur Saat Mendaftar", min_value=0, max_value=80, value=0)

    with col3:
        mothers_occupation = st.selectbox("Pekerjaan Ibu", ["Pilih", 'Unemployed', "Student", "Legislative/Executive/Directors", "Intellectual/Scientific","Technicians", "Administrative staff", "Services/Security/Sellers","Farmers/Fishers", "Industry/Construction", "Machine Operators","Unskilled Workers", "Armed Forces", "Other", "(blank)","Health professionals", "Teachers", "ICT specialists", "Intermediate science/engineering technicians", "Intermediate health professionals","Legal/social/cultural technicians", "Secretaries/data processing","Accounting/financial/registry operators", "Other admin support","Personal service workers","Sellers", "Personal care workers","Construction workers", "Jewelers/artisans/etc.", "Food/clothing/woodworking","Cleaning workers", "Unskilled agri/fish/forestry", "Unskilled extractive/construction/etc.", "Meal prep assistants", "Other"])
        fathers_occupation = st.selectbox("Pekerjaan Ayah", ["Pilih", 'Unemployed', "Student", "Legislative/Executive/Directors", "Intellectual/Scientific","Technicians","Administrative staff", "Services/Security/Sellers", "Farmers/Fishers","Industry/Construction", "Machine Operators", "Unskilled Workers","Armed Forces", "Other", "(blank)","Armed Forces Officers","Armed Forces Sergeants", "Other Armed Forces personnel","Admin/Commercial Directors", "Hotel/Catering Directors", "Science/Engineering Specialists","Health professionals", "Teachers", "Finance/Admin Specialists","Intermediate science/engineering technicians", "Intermediate health professionals","Legal/social/cultural technicians", "ICT technicians","Secretaries/data processing","Accounting/financial/registry operators", "Other admin support","Personal service workers", "Sellers", "Personal care workers","Protection/security personnel", "Skilled agricultural producers","Subsistence farmers/fishers", "Skilled construction workers","Metalworkers", "Electricians","Food/clothing/woodworking","Fixed plant operators", "Assembly workers", "Drivers and mobile equipment operators","Unskilled agri/fish/forestry", "Unskilled extractive/construction/etc.","Meal prep assistants", "Street vendors","Other"])
        gender = st.selectbox("Jenis Kelamin", ["Pilih", 'Male', 'Female'])
        international = st.selectbox("Mahasiswa Internasional", ["Pilih", "Yes", "No"])

    st.markdown("## ⚙️ Kondisi Sosial & Finansial")
    col4, col5, col6 = st.columns(3)
    with col4:
        displaced = st.selectbox("Displaced", ["Pilih", "Yes", "No"])
    with col5:
        special_needs = st.selectbox("Berkebutuhan Khusus", ["Pilih", "Yes", "No"])
    with col6:
        debtor = st.selectbox("Punya Tunggakan?", ["Pilih", "Yes", "No"])

    col7, col8 = st.columns(2)
    with col7:
        tuition_paid = st.selectbox("Pembayaran Lancar?", ["Pilih", "Yes", "No"])
    with col8:
        scholarship = st.selectbox("Penerima Beasiswa", ["Pilih", "Yes", "No"])

    st.markdown("## 📘 Semester 1")
    col9, col10, col11 = st.columns(3)
    with col9:
        cu1_credited = st.number_input("Kredit Diterima (S1)", min_value=0, max_value=50, value=0)
        cu1_approved = st.number_input("Disetujui (S1)", min_value=0, max_value=50, value=0)
    with col10:
        cu1_enrolled = st.number_input("Mata Kuliah Diambil (S1)", min_value=0, max_value=50, value=0)
        cu1_grade = st.number_input("Nilai Rata-rata (S1)", min_value=0.0, max_value=100.0, value=0.0)
    with col11:
        cu1_eval = st.number_input("Evaluasi (S1)", min_value=0, max_value=20, value=0)
        cu1_no_eval = st.number_input("Tanpa Evaluasi (S1)", min_value=0, max_value=20, value=0)

    st.markdown("## 📙 Semester 2")
    col12, col13, col14 = st.columns(3)
    with col12:
        cu2_credited = st.number_input("Kredit Diterima (S2)", min_value=0, max_value=50, value=0)
        cu2_approved = st.number_input("Disetujui (S2)", min_value=0, max_value=50, value=0)
    with col13:
        cu2_enrolled = st.number_input("Mata Kuliah Diambil (S2)", min_value=0, max_value=50, value=0)
        cu2_grade = st.number_input("Nilai Rata-rata (S2)", min_value=0.0, max_value=100.0, value=0.0)
    with col14:
        cu2_eval = st.number_input("Evaluasi (S2)", min_value=0, max_value=20, value=0)
        cu2_no_eval = st.number_input("Tanpa Evaluasi (S2)", min_value=0, max_value=20, value=0)

    st.markdown("---")
    submitted = st.form_submit_button("🔍 Prediksi")

if submitted:
    if "Pilih" in [marital_status,application_mode,course,attendance,prev_qualification, mothers_occupation,fathers_occupation,gender,international,displaced,special_needs,debtor,tuition_paid,scholarship]:
        st.warning("⚠️ Harap lengkapi semua data terlebih dahulu.")
    else:
